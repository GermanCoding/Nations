...
public class VisibilityCooldownTask implements Runnable {
	...
	private static ArrayList<VisibilityCooldownTask> instances = new ArrayList<VisibilityCooldownTask>();
	...
	public static boolean isInCooldown(Player p) {
		synchronized (mainLock) {
			for (VisibilityCooldownTask t : instances) {
	...
	public VisibilityCooldownTask(Player p, int seconds) {
		this.p = p;
		if (!isInCooldown(p)) {
			synchronized (mainLock) {
				instances.add(this);
